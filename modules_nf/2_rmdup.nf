...
process DEREPLICATE_SEQS {
    ...
    publishDir "${params.outdir}/02_seqkit/filtered_genomes", pattern: "unique_genomes/*", mode: 'copy', saveAs: { filename -> filename.split('/').last() }
    publishDir "${params.outdir}/02_seqkit", pattern: "rmdup_report.txt", mode: 'copy'
    publishDir "${params.outdir}/02_seqkit", pattern: "dup_sequences.fasta", mode: 'copy'
