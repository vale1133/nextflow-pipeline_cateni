...
process BAKTA_ANNOTATION {
    ...
    publishDir "${params.outdir}/5_bakta/${meta.id}", mode: 'copy', saveAs: { filename -> filename.split('/').last() }
    ...
    output:
    path "bakta_output/*.gff3", emit: gff
    path "bakta_output/*.json", emit: json
    tuple val(meta), path("bakta_output/${meta.id}.faa") , emit: proteins
    tuple val(meta), path("bakta_output/${meta.id}.hypothetical.faa") , emit: hypothetical_proteins
